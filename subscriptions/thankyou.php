...
<h2 class="text-center"> Your subscription order has been placed successfully !!! <br> <br>Thank you for your order <br><br> You can track your order at <strong>Your Orders</strong> Page</h2>
</div>
<?php $c_id = $_SESSION['c_id'];
          $grand_total = '';
          $check_sql = "SELECT * FROM subscription WHERE sub_cust_id = '$c_id' ORDER BY subscribed ASC LIMIT 1 ";
            $check_run_query = mysqli_query($con, $check_sql);
              ...
              $sub_id = $cart['subscription_id'];
               $frequency = $cart['sub_frequency'];
                $date_now = date("Y-m-d");
              $next_due = date('Y-m-d', strtotime("+$frequency months", strtotime($date_now)));
            // $con->query("UPDATE subscription SET subscribed = '1', sub_next_due = '$next_due' WHERE subscription_id = '$sub_id' ");
