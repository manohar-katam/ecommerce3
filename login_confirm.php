...
if(empty($signinEmail) || empty($signinPassword)){
	...
	$signinEmail = mysqli_real_escape_string($con, $_POST["signinEmail"]); // Protecting from hackers
	$signinPassword = md5($_POST["signinPassword"]);  //converting password to md5 format

	$sql = "SELECT * FROM customers WHERE customer_email = '$signinEmail' AND customer_password = '$signinPassword' AND customer_type = '0'";
